...
class Service:
    ...
    def processCreditNote(self, data): 
        ...
        try:
            ...
            exc_traceback = sys.exc_info()
            # with open('/var/lib/odoo/addons/dianservice-1.0.43/log.js', 'w') as outfile:
            #     json.dump(getattr(e, 'message', repr(e))+" ON LINE " + format(sys.exc_info()[-1].tb_lineno), outfile)
...
#
    ...
    def doRequestDian(self,config, data, _type=None):
        ...
        try:
            ...
            if data["ambiente_ejecucion"] == '1':  # Producción
                config['dian_webservice_url'] = 'https://vpfe.dian.gov.co/WcfDianCustomerServices.svc'
                if(_type=="attached_document"):
                    response = service.send_bill_attachement_async(
            ...
            if(_type=="attached_document"):
                return self.format_AD_response(response, service, data)
            else:
    ...
    def format_AD_response(self, response_text,service, data):
        response = {"status":"FAIL","body":"DIAN-Attached Document: Servidor no disponible temporalmente."}
        try:
            if service.get_response_status_code() == 200:
                xml_nodes = etree.fromstring(response_text)
                tree_nodes = xml_nodes.getroottree()
                SendBillAttachmentAsyncResponse = tree_nodes.getroot()
                for ErrorMessageList in SendBillAttachmentAsyncResponse.iter(tag='{http://schemas.datacontract.org/2004/07/UploadDocumentResponse}ErrorMessageList'): 
    ...
    def formatDianResponse(self, response_text,service, data):
        #with open('/var/lib/odoo/addons/12.0/dian_efact/dianservice/log.js', 'w') as outfile:
        #    json.dump(str(response_text), outfile)
    ...
    def saveSendBillResponse1(self, xmlResponse):
        ...
        return xmlTarget
                                                      #get_status
    def saveAttachedDocuments(self, xmlResponse, _type="attached_document" ):
        xmlTarget = str(self.xmlPath+"/XMLresponses/"+self.xmlClientPath+'/')+ str(_type) + str("_") + str(self.fileXmlName)+".xml"
        with open(xmlTarget, "w") as f:
            f.write(xmlResponse)
        return xmlTarget
    ...
    def getTrackId(self):
        ...
        cufe = UUID.text
        cufe_scheme = UUID.get("schemeName")
        ...
        trackID = {"cufe":cufe, "cufe_scheme":cufe_scheme, "qr_url":qr_url}
        return trackID
    ...
    def getStatus(self,data,cufe):
        ...
        docs.setXMLPath(self.xmlPath)
        config =    {
                        'dian_webservice_url':'https://vpfe.dian.gov.co/WcfDianCustomerServices.svc',
                        'policy_id': "https://facturaelectronica.dian.gov.co/politicadefirma/v2/politicadefirmav2.pdf",
                        'policy_name': "Política de firma para facturas electrónicas de la República de Colombia",
                        'policy_remote': docs.Base64EncodeAnyDocument(self.xmlPath+"/politicadefirmav2.pdf"),
                        'key_file': docs.Base64EncodeAnyDocument(self.xmlPath+"/XMLcertificates/"+self.xmlClientPath+"/"+data['dian']['certificado']),
    ...
    def processAttachedDocument(self,data):
        docs = Documents()
        _response = {}
        response = {}
        try:
            ...
            status_response = self.getStatus(data, track_id["cufe"])
            self.saveAttachedDocuments(status_response,"get_status")
            _response = self.getAppResponseInfo(track_id)
            docs.fillDocument("attachedDocument", data, _response) 
            config = {
                            'dian_webservice_url':'https://vpfe-hab.dian.gov.co/WcfDianCustomerServices.svc',
                            'policy_id': "https://facturaelectronica.dian.gov.co/politicadefirma/v2/politicadefirmav2.pdf",
                            'policy_name': "Política de firma para facturas electrónicas de la República de Colombia",
                            ...
                            'key_file': docs.Base64EncodeAnyDocument(self.xmlPath+"/XMLcertificates/"+self.xmlClientPath+"/"+data['dian']['certificado']),
                            'key_file_password': data["dian"]["certificado_contrasena"],
                            'filename': docs.documentXmlName+str("_attached_document.zip")
                        }
                
            docs.signDocument(config, "attached_document")
            
            if(data['accion']=="fill_submit"):
                wasZipped = docs.zipDocument("attached_document")
                if (wasZipped==True):
                    zipFileEncoded = docs.Base64EncodeDocument("attached_document")
                    config ['zipped_file'] = zipFileEncoded                
                    response = self.doRequestDian(config, data, "attached_document")
        ...
        _response['xml_signed'] = docs.getXMLSignedDocument("attached_document")
        _response['xml_unsigned'] = docs.getXMLUnsignedDocument("attached_document")
        return response

    def getAppResponseInfo(self, track_id):
        # read attached_document
        xmlDocStatus = str(self.xmlPath+"/XMLresponses/"+self.xmlClientPath+'/')+ str('get_status') + str("_") + str(self.fileXmlName)+".xml"
        _response = {'IssueDate':None, 'IssueTime':None, 'SenderParty':None, 'ReceiverParty':None, 'Response':None, 'DocumentReference':None}
        try:
            tree = etree.parse(xmlDocStatus)
            MainNode = tree.getroot()
            
            for Body in MainNode.iter("{http://www.w3.org/2003/05/soap-envelope}"+"Body"):
                
                for GetStatusResponse in Body.iter("{http://wcf.dian.colombia}"+"GetStatusResponse"):
                    
                    for GetStatusResult in GetStatusResponse.iter("{http://wcf.dian.colombia}"+"GetStatusResult"):                                 
                        StatusCode = GetStatusResult.find("{http://schemas.datacontract.org/2004/07/DianResponse}"+"StatusCode")
                        XmlBase64Bytes = GetStatusResult.find("{http://schemas.datacontract.org/2004/07/DianResponse}"+"XmlBase64Bytes")
                        
                        if(str(StatusCode.text) == "00"):
                            xml_decoded = base64.b64decode(str(XmlBase64Bytes.text))
                            xml_nodes = etree.fromstring(xml_decoded)
                            tree_nodes = xml_nodes.getroottree()

                            ApplicationResponse = tree_nodes.getroot()
                            IssueDate = ApplicationResponse.find("{urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2}"+"IssueDate")
                            _response['IssueDate'] = IssueDate
                            IssueTime = ApplicationResponse.find("{urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2}"+"IssueTime")
                            _response['IssueTime'] = IssueTime

                            SenderParty = ApplicationResponse.find("{urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2}"+"SenderParty")
                            _response['SenderParty'] = SenderParty
                            ReceiverParty = ApplicationResponse.find("{urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2}"+"ReceiverParty")
                            _response['ReceiverParty'] = ReceiverParty

                            for DocumentResponse in ApplicationResponse.iter("{urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2}"+"DocumentResponse"): 
                                Response = DocumentResponse.find("{urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2}"+"Response")
                                _response['Response'] = Response
                                DocumentReference = DocumentResponse.find("{urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2}"+"DocumentReference")                            
                                _response['DocumentReference'] = DocumentReference
                                

            _response['track_id'] =  track_id            
        except Exception as e:            
            return _response
        return _response