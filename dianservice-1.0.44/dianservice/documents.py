...
from . attached_documents import AttachedDocuments
...
class Documents:
    ...
    def fillDocument(self, whichDocument, data, app_reponse=None):
        ...
        if(whichDocument=="attachedDocument"):
           attached_documents = AttachedDocuments()
           fileFilled = attached_documents.fillDocument(self.xmlPath, self.xmlClientPath,self.documentXmlName, data, app_reponse)
    ...
    def signDocument(self, config, _type=None):
        try:
            signed_xml = DIANXMLSigner().sign(self.getXMLUnsignedDocument(_type), config)  
            if(_type=="attached_document"):
                signed_path = self.xmlPath+"/XMLdocuments/3_signed/"+self.xmlClientPath+"/"+self.documentXmlName+"_attached_document.xml"         
            else:
            ...
            #with open('/odoo_rockscripts/custom/addons/dianservice-1.0.35/log.js', 'w') as outfile:
    ...
    def zipDocument(self, _type=None):
        try:      
            if(_type=="attached_document"):
                xmlSource = str(self.xmlPath+"/XMLdocuments/3_signed/"+self.xmlClientPath+'/')+str(self.documentXmlName)+str("_attached_document.xml")
                zipTarget = str(self.xmlPath+"/XMLdocuments/4_compressed/"+self.xmlClientPath+'/')+str(self.documentZipName)+"_attached_document.zip"            
            else:     
    ...
    def Base64EncodeDocument(self, _type=None):
        if(_type=="attached_document"):
            fileContents = open(self.xmlPath+'/XMLdocuments/4_compressed/'+self.xmlClientPath+"/"+str(self.documentZipName)+"_attached_document.zip", "rb").read()
        else:
    ...
    def getXMLUnsignedDocument(self, _type=None):
        try:
            if(_type=="attached_document"):           
                fileContents = open(self.xmlPath+'/XMLdocuments/2_unsigned/'+self.xmlClientPath+"/"+str(self.documentXmlName)+"_attached_document.xml", "rb").read()
            else:
    ...
    def getXMLSignedDocument(self, _type=None):
        if(_type=="attached_document"): 
            fileContents = open(self.xmlPath+'/XMLdocuments/3_signed/'+self.xmlClientPath+"/"+str(self.documentXmlName)+"_attached_document.xml", "rb").read()
        else:
