...
class Wsdl(object):
    ...
    def _prepare_header_and_sign(self):
        try:
            ...
            if sys.version_info<(3,6,0):
                created_time = datetime.datetime.now(pytz.timezone('UTC'))
            else:
    ...
    def send_bill_attachement_async(self, zip_name, zip_data):
        try:
            self._prepare_header_and_sign()
            soap = self._xml_base_template

            soap.xpath('//soap:Header/wsa:Action', namespaces=nsd)[0].text = actions['SendBillAttachmentAsync']
            body = etree.Element(etree.QName(nsd['soap'], 'Body'), nsmap={'soap': nsd['soap'], 'wcf': nsd['wcf']})
            send_bill_async_tag = etree.Element(etree.QName(nsd['wcf'], 'SendBillAttachmentAsync'))
            file_name = etree.Element(etree.QName(nsd['wcf'], 'fileName'))
            content_file = etree.Element(etree.QName(nsd['wcf'], 'contentFile'))
